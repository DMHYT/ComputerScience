# Дополнительное задание к уроку "Алгоритмы обработки списков. Алгоритмы сортировки Bubble Sort и Quick Sort"
# Ничем не отличается от improved-версии, только добавлен вывод в консоль
# времени в миллисекундах, за которое выполнилась сортировка.
# Таким образом можно увидеть, какой из изученных алгоритмов быстрее и эффективнее,
# ведь, хоть кому-то оба варианта покажутся космически быстрыми, в компьютере имеет значение каждая наносекунда)))

import tkinter as tk
import tkinter.messagebox as mb
# Импортируем метод time из библиотеки time. Он возвращает количество секунд,
# прошедших с 1 января 1970 года. Мы будем вызывать его дважды и высчитывать разницу,
# таким образом получая время, потраченное на выполнение программы
import time
...
def sort_box():
    ...
    box.delete(0, 'end')
    # Получаем время до операций
    current = time.time()
    if mode.get() == 0:
        bubble_sort(array)
    else:
        array = quick_sort(array)
    # Вычисляем разность и умножаем на 1000, таким образом переводя в миллисекунды
    diff = (time.time() - current) * 1000
    # Выводим результат в консоль
    print("Алгоритм " + ("Bubble Sort" if mode.get() == 0 else "Quick Sort") + " выполнен успешно за " + str(diff) + " мс")
    for item in array:
        box.insert('end', item)

# Как мы сможем увидеть, Quick Sort работает в несколько раз быстрее Bubble Sort
